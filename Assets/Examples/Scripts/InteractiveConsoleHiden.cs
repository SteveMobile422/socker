...
public sealed class InteractiveConsoleHiden : MonoBehaviour
{
	
	public		BitmapMeshText		debuger;
    ...
    public void CallFBFeed()
    {
		...
		starsCollected = AllStarsCollected();
		if(PlayerPrefs.GetString("Version", "premium") == "premium")
			FeedLink = "https://itunes.apple.com/us/app/kick-the-ball!-premium/id723995025?ls=1&mt=8";
		else
		FeedLink = "https://itunes.apple.com/us/app/kick-the-ball!-free/id721400933?ls=1&mt=8";
		
		FeedLinkCaption ="";
		FeedLinkDescription = "";
		FeedLinkName = "I'm just playing KICK THE BALL! It's like Angry Birds for football fans and I already collected " + starsCollected +" stars, can you beat me?";
	...
	private int AllStarsCollected(){
		gameControler gameKontroler = GameObject.FindGameObjectWithTag("GameController").GetComponent<gameControler>();
		
		int starsColected = 0;
		starsColected += gameKontroler.AllStarsForLevel(1);
		starsColected += gameKontroler.AllStarsForLevel(2);
		starsColected += gameKontroler.AllStarsForLevel(3);
		starsColected += gameKontroler.AllStarsForLevel(4);
		starsColected += gameKontroler.AllStarsForLevel(5);
			
		return starsColected;
